...
def apex_prompt(message: str) -> str:
    ...
    return (
        ...
        "If your conclusion is a BUY or SELL, STAGE the order (do not place it): "
        "record the decision in the journal, then run `hf-bot order propose "
        "--decision <id>` so a sized proposal appears in the principal's Orders "
        "panel for approval. Never approve or place it yourself — placement is "
        "the principal's explicit action, paper money only, behind the kill "
        "switch.\n\n"
